...
export const PromptBox = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement> & {
...
}>(({ className, onSubmitMessage, ...props }, ref) => {
  ...
  const handleRecordToggle = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          const res = await axios.post('http://localhost:4646/api/speech', {
            audioBase64: base64.replace(/^data:.*;base64,/, ''),
            mimeType: blob.type,
            languageCode: 'tr-TR'
          });
  ...
  return (
    <div className={cn("flex flex-col rounded-[28px] p-2 shadow-sm transition-colors bg-white/5 border border-neutral-700 cursor-text", className)}>
      ...
      <textarea name="message" ref={internalTextareaRef} rows={1} value={value} onChange={handleInputChange} placeholder="Mesaj..." className="custom-scrollbar w-full resize-none border-0 bg-transparent p-3 text-neutral-100 placeholder:text-neutral-400 focus:ring-0 focus-visible:outline-none min-h-12" {...props} />
            ...
            <div className="ml-auto flex items-center gap-2">
              <Tooltip>
                <TooltipTrigger asChild>
                  <button type="button" onClick={handleRecordToggle} className={cn("flex h-8 w-8 items-center justify-center rounded-full transition-colors focus-visible:outline-none", isRecording ? 'bg-red-500/90 text-white hover:bg-red-500' : 'text-neutral-100 hover:bg-white/10')}>
                    <MicIcon className="h-5 w-5" />
                    <span className="sr-only">Ses kaydı</span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="top" showArrow={true}><p>Sesle yaz</p></TooltipContent>
              </Tooltip>
              <button type="button" onClick={handleSend} disabled={!hasValue} className="relative inline-block py-[1px] overflow-hidden rounded-[20px] disabled:opacity-50">
                <div className="absolute w-[300%] h-[50%] bottom-[-11px] right-[-250%] rounded-full animate-star-movement-bottom z-0 opacity-20" style={{ background: 'radial-gradient(circle, white, transparent 10%)', animationDuration: '6s' }} />
                <div className="absolute w-[300%] h-[50%] top-[-10px] left-[-250%] rounded-full animate-star-movement-top z-0 opacity-20" style={{ background: 'radial-gradient(circle, white, transparent 10%)', animationDuration: '6s' }} />
                <div className="relative z-1 border text-center text-base py-2 px-4 rounded-[20px] bg-white text-black">
                  Gönder
                </div>
              </button>
            </div>
